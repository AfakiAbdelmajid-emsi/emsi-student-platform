...
/**
 * Handles image upload to your backend API
 * @param file The file to upload
 * @param noteId The ID of the note the image belongs to
...
export const handleImageUpload = async (
  ...
  try {
    // Use your API client for the upload
    const response = await client<{
      ...
      body: formData,
      headers: {
        // Authorization handled automatically by your client
      },
      signal: abortSignal,
    });
    const imageUrl = `${process.env.NEXT_PUBLIC_SUPABASE_URL}/storage/v1/object/public/filesb/${response.file_data.file_path}`;
